    ...
    <section className="card border-0 shadow-sm">
      <div className="card-body">
        <div className="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-3">
          <h2 className="h3 text-primary mb-0">Teams</h2>
        ...
        {loading && <div className="alert alert-info py-2">Loading teams...</div>}
              ...
              {filteredTeams.length === 0 ? (
                ...
                filteredTeams.map((team, index) => (
                  <tr key={team.id || team._id || index}>
                    <td>{index + 1}</td>
                    <td className="fw-semibold">{team.name || team.team_name || 'Team'}</td>
                    <td>
                      Members: {Array.isArray(team.members) ? team.members.length : team.member_count || 0}
                    </td>
